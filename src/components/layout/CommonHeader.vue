  ...
  <div class="header clearfix" v-if="!isHideHeader">
    <div class="common_header-title" :class="{ iphone: isIOS && isInApp }">
      ...
      <a href="javascript:;" @click="goBack" class="back">
        <img class="arrow" src="../../assets/images/store/store_i_arrow.png" alt="">
      </a>

      <!-- 标题 -->
      <div class="title row-ellipsis">
        {{ title }}
      </div>

      <!-- 分享按钮 -->
      <a href="javascript:;" v-if="share && isInApp" @click="handleBubbleShow" class="btn_share"></a>
      <div v-else class="options" @click="$emit('handleRight')">
        <slot name="right-options"></slot>
      </div>
      <!-- 筛选按钮 -->
      <a href="javascript:;" v-if="filter" @click="handleFilter" class="btn_filter btn"></a>
      <!-- 回看历史按钮 -->
      <a href="javascript:;" v-if="history" @click="handleHistory" class="btn_history btn"></a>
      <!-- 新增按钮 -->
      <a href="javascript:;" v-if="hasAdd" @click="handleAdd" class="btn_add btn"></a>
      <!-- 头部右侧地址 -->
      <a href="javascript:;" v-if="hasBtn" @click="handleAdd" class="btn name">{{btnName}}</a>
    </div>

    <!-- 分享盒子 -->
    <transition-group enter-active-class="animated fadeIn" leave-active-class="animated fadeOut">
      <ul key="share-box" class="common_bubble_box clearfix" :class="{'iphone': isIOS}" v-if="isBubble">
...
.common_header-title {
  ...
  .btn_share {
    ...
    width: 1.1rem;
    height: 1rem;
    background: url("~images/common/common_i_share.png") no-repeat center;
